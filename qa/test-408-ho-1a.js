...
function fakeForm(campaign, source, segment) {
  const values = {
    first_name: 'Dylan',
    last_name: 'Haysbert',
    phone: '(408) 327-6377',
...
for (const page of handoffPages) {
  test(`${page.rel} loads the canonical profile pipeline`, () => {
    ...
    assert.equal(attribute(html, 'data-sender-build'), '408-HO-1G');
    assert.equal(attribute(html, 'data-handoff-contract'), 'coveragefit-handoff-v1');

    const configIndex = html.indexOf('../shared/config.js');
    const launcherIndex = html.indexOf('../shared/coveragefit-launch.js');
    const profileIndex = html.indexOf('../shared/prospect-profile.js');
    const formIndex = html.indexOf('../shared/script.js');
  ...
  test(`${page.rel} produces a personalized CoverageFit URL`, () => {
    ...
    assert.equal(url.searchParams.get('sender_build'), '408-HO-1G');
    ...
    assert.equal(url.searchParams.get('segment'), 'Current policy renewal');
...
test('auto-bundle remains intentionally outside the CoverageFit handoff', () => {
