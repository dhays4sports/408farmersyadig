...
check('stable contract is exposed by every personalized form', () => {
  for (const rel of ['home/index.html','tech/index.html','engineers/index.html','healthcare/index.html','teachers/index.html']) {
    const html=read(rel); assert(html.includes('data-sender-build="408-HO-1G"')); assert(html.includes('data-handoff-contract="coveragefit-handoff-v1"'));
  }
});
...
check('manifest identifies stable schema and current CoverageFit receiver', () => {
  assert.equal(manifest.build,'408-HO-1G'); assert.equal(manifest.handoffContract,'coveragefit-handoff-v1'); assert.equal(manifest.receiver,'CoverageFit v3.20.8');
});
...
console.log(`\n408-HO-1G QA: ${checks.length}/${checks.length} passed`);
